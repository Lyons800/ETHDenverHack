...
contract SemanticSwap {
    ...
    function proposeMatch(string memory reason, MatchLinkAddresses[] memory matchLinks) public {
        MatchProposal prop = new MatchProposal(reason);
        
        for (uint256 i = 0; i < matchLinks.length; i++) {
            Order producer = Order(matchLinks[i].producer);
            Order consumer = Order(matchLinks[i].consumer);
